  ...
  window.ScreenSummary = function (ev) {
    ...
    return UI.subbar('Proposed Solution', '#/eval/' + ev.id + '/hub') +
      ...
      (fin.items.length === 0 ?
        ...
        '<button class="btn dark" data-action="proposal-pdf">Generate Proposal PDF ' + icon('print') + '</button>' +
        '</div>') +
      '</div>';
  };

  /* ---------------- Media Review & Tagging ---------------- */
  window.ScreenMedia = function (ev, filterZone) {
    ...
    var grid = '<div class="media-grid">' + shown.map(function (p) {
      ...
      if (!url) return '';
      return '<div class="media-tile"><img src="' + url + '" alt="' + esc(p.label || 'Photo') + '">' +
        '<div class="meta">' + esc(p.label || 'Photo') + '<br>' + esc((p.ts || '').slice(0, 10)) + ' · ' + esc(p.inspector || '') + '</div>' +
        '<button class="retake" style="top:8px;right:8px;position:absolute" data-action="photo-remove" data-photo="' + p.id + '">' + icon('trash') + '</button>' +
        '</div>';
    }).join('') + '</div>';

    return UI.subbar('Media Review', '#/eval/' + ev.id + '/hub',
        '<button class="iconbtn" data-action="toast" data-msg="Batch export lands with cloud sync." aria-label="Export">' + icon('export') + '</button>') +
      '<div class="screen">' +
      '<h1 class="screen-title">Assessment Media</h1>' +
      '<p class="screen-sub">Captured media for residential energy systems and compliance documentation.</p>' +
      chips +
      (shown.length ? grid :
  ...
  window.ScreenProposalMedia = function (ev) {
    ...
    return UI.subbar('Proposal Media', '#/eval/' + ev.id + '/summary') +
      ...
      (sel.length ?
        ...
        '<button class="btn primary" data-action="proposal-pdf">Review ' + icon('chevR') + '</button></div>' : '') +
      '</div>';
  };
